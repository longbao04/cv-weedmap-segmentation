...
植被覆盖率 VCR 是模型选择前置指标：`analyze_vegetation_coverage.py` 使用 common split 的 NDVI 和有效区域 mask，初步区分稀疏、过渡、密集场景，供选择 YOLO 或 U-Net 时参考。当前 VCR 阈值 0.20/0.30 是初始经验阈值，后续需根据验证集分布和实际除草需求调整。
